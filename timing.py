...
for i in range(10, 510, 10):
    seq1 = random_nucleotide(32)
    seq2 = random_nucleotides(i)
    ...
    # for seq in seq2:
    #     dp = wagnerFisher(seq1, seq['sequence'])
    # end_time = time.time()
    ...
    start_time = time.time()
    vec1 = convert_to_idf_vector(seq1, list_of_docs=seq2, doc_count=i)
    vec2 = []
    for seq in seq2:
    ...
    start_time = time.time()
    for s in set2:
        set_intersection_similarity(set1, s)
    end_time = time.time()
    ...
    start_time = time.time()
    for m in multi2:
        multi_intersection_similarity(multi1, m)
    end_time = time.time()
    ...
    start_time = time.time()
    for m in multi2:
        multi_jaccard_similarity(multi1, m)
    end_time = time.time()
    ...
    start_time = time.time()
    for m in multi2:
        multi_dice_similarity(multi1, m)
    end_time = time.time()
    ...
    start_time = time.time()
    for second in vec2:
        euclidian_distance(vec1, second)
    end_time = time.time()
    ...
    start_time = time.time()
    for second in vec2:
        manhattan_distance(vec1, second)
    end_time = time.time()
...
# ax.set(title='Wagner Fischer', xlabel='# Of Sequences', ylabel='Time (ms)')
...
# fig = plt.figure(figsize=(6,6))
# ax = fig.add_subplot(111)
# ax.set(title='IR Conversion', xlabel='# Of Sequences', ylabel='Time (ms)')
...
fig = plt.figure(figsize=(6,6))
ax = fig.add_subplot(111)
ax.set(title='Set Methods', xlabel='# Of Sequences', ylabel='Time (ms)')
...
fig = plt.figure(figsize=(6,6))
ax = fig.add_subplot(111)
ax.set(title='Multiset Methods', xlabel='# Of Sequences', ylabel='Time (ms)')
...
# fig = plt.figure(figsize=(6,6))
# ax = fig.add_subplot(111)
# ax.set(title='Vector Methods', xlabel='# Of Sequences', ylabel='Time (ms)')
...
# plt.grid()
# plt.ylim(top=0.005)
# plt.savefig('./vector_methods_1.png')
# plt.show()
